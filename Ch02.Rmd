...
The number of microstates for a particular macrostate is given as: $\binom{n}{k} = \frac{n!}{x!(n-x)!}$, where $n$ is the number of coins and $x$ is the number of times you receive heads. Assuming you have 20 coins, how many microstates are there for receiving 10 times heads?
...
Consider a sample of ice with a mass of 3 kg that is melts at a constant temperature of about 273.15K. Calculate the corresponding change in entropy. Assume that the energy required for melting (i.e. the heat of fusion) equals `r 3.34 * 10^5` J/kg.
